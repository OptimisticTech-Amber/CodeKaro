...
function EditorPanel({ selectedFile, code, onChange }: ScreenOneProps) {
  ...
  useEffect(() => {
    ...
    setLanguages(getLanguageForFile(selectedFile.name));
  }, [selectedFile]);
  ...
  useEffect(() => {
    const savedCode = localStorage.getItem(`editor-code-${languages}`);
    const newCode = savedCode || languages;
  ...
  const handleRefresh = () => {
    const defaultCode = languages;
    if (editor) editor.setValue(defaultCode);
    localStorage.removeItem(`editor-code-${languages}`);
  };

  const handleEditorChange = (value: string | undefined) => {
    if (value) localStorage.setItem(`editor-code-${languages}`, value);
  };
