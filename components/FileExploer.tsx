...
const [files, setFiles] = useState<FileData[]>(initialFiles);
  ...
  const handleDeleteFile = async (e: React.MouseEvent, id: string) => {
    e.stopPropagation();
    setFiles((prevFiles) => prevFiles.filter((file) => file.id !== id)); // Optimistic update
          ...
          {files.map((file) => (
            ...
            >
              {editingFile === file.id ? (
                ...
                onBlur={(e: React.FocusEvent<HTMLInputElement>) => handleEditSave(e, file.id)}
                onKeyDown={(e: React.KeyboardEvent<HTMLInputElement>) =>
                e.key === "Enter" ? handleEditSave(e, file.id) : null
                }
                  autoFocus
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                onClick={(e: React.MouseEvent) => handleDeleteFile(e, file.id)}
